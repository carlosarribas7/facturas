...
// --------- orden para las cabeceras ------
$camposOrdenables = [
    'f.id', 'proveedor_nombre', 'tipoCuenta', 'tipoCE', 
    'retencion_valor', 'retencion_ambito', 
    'pago_nombre', 'f.numero_factura', 'f.fecha_factura', 'f.total_factura'
];
$orden = in_array($_GET['orden'] ?? '', $camposOrdenables) ? $_GET['orden'] : 'f.id';
$tipo = ($_GET['tipo'] ?? 'DESC') === 'ASC' ? 'ASC' : 'DESC';
...
$listaProveedores = $pdo->query("SELECT id, nombre, tipoGasto_id FROM proveedores ORDER BY nombre ASC")->fetchAll(PDO::FETCH_ASSOC);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // required basic validation (numero_factura and proveedor_id)
    ...
    if (isset($_POST['id']) && $_POST['id'] !== '') {
        ...
        $sql = "UPDATE facturas SET proveedor_id=?, tipoGasto_id=?, numero_factura=?, fecha_factura=?, mes=?,
                ...
                retencion=?, retencion_id=?, total_factura=?, pago_id=?, fecha_vencimiento=?, fecha_banco=?
                WHERE id=?";
        ...
        $stmt->execute([
            ...
            $data['pago_id'], $data['fecha_vencimiento'], $data['fecha_banco'],
            $_POST['id']
        ]);
    } else {
        // INSERT
...
<div class="flex justify-between items-center mb-6">
    ...
    <div class="flex items-center space-x-3">
        <a href="exportar_excel.php" class="bg-blue-600 text-white font-semibold px-4 py-2 rounded hover:bg-blue-700 flex items-center">
            ...
            <tr>
                <th class="border px-3 py-2">
                    <a href="?<?php 
                        $params = $_GET;
                        $params['orden'] = 'f.id';
                        $params['tipo'] = ($orden === 'f.id' && $tipo === 'ASC') ? 'DESC' : 'ASC';
                        $params['pagina'] = 1; // Reset to first page
                        echo http_build_query($params);
                    ?>">
                        ID
                        <?php if ($orden === 'f.id'): ?>
                            <i class="fas fa-sort-<?= $tipo === 'ASC' ? 'up' : 'down' ?> ml-1"></i>
                        <?php else: ?>
                ...
                <th class="border px-3 py-2">
                    <a href="?<?php 
                        $params = $_GET;
                        $params['orden'] = 'proveedor_nombre';
                        $params['tipo'] = ($orden === 'proveedor_nombre' && $tipo === 'ASC') ? 'DESC' : 'ASC';
                        $params['pagina'] = 1; // Reset to first page
                        echo http_build_query($params);
                    ?>">
                        ID Proveedor
                        <?php if ($orden === 'proveedor_nombre'): ?>
                            <i class="fas fa-sort-<?= $tipo === 'ASC' ? 'up' : 'down' ?> ml-1"></i>
                        <?php else: ?>
                            <i class="fas fa-sort ml-1 text-gray-400"></i>
                        <?php endif; ?>
                ...
                <th class="border px-3 py-2">
                    <a href="?<?php 
                        $params = $_GET;
                        $params['orden'] = 'tipoCuenta';
                        $params['tipo'] = ($orden === 'tipoCuenta' && $tipo === 'ASC') ? 'DESC' : 'ASC';
                        $params['pagina'] = 1; // Reset to first page
                        echo http_build_query($params);
                    ?>">
                        Tipo Gasto
                        <?php if ($orden === 'tipoCuenta'): ?>
                            <i class="fas fa-sort-<?= $tipo === 'ASC' ? 'up' : 'down' ?> ml-1"></i>
                        <?php else: ?>
                            <i class="fas fa-sort ml-1 text-gray-400"></i>
                        <?php endif; ?>
    ...
    <div class="bg-white rounded-lg p-7 shadow-lg w-full max-w-7xl" 
        ...
        <form method="POST" @submit.prevent="submitForm($event)">
            ...
            <div class="grid grid-cols-5 gap-4 mb-4">
                ...
                <div>
                    ...
                    <select name="proveedor_id" x-model="form.proveedor_id"
                        @change="cargarTipoGastoProveedor()"
            ...
            <div class="grid grid-cols-5 gap-4 mb-4">
                ...
                <div>
                    ...
                    <input name="bi1" type="text" 
                        x-model="form.bi1" 
                        @input="calcularCI(); if(form.retencion_id) calcularRetencion(); else calcularTotal();"
                        class="border px-2 py-1 w-full rounded">
                </div>
                ...
                <div>
                    ...
                    <select name="iva1" 
                            x-model="form.iva1"
                            @change="calcularCI(); calcularTotal()"
                            class="border px-2 py-1 w-full rounded">
            ...
            <div class="grid grid-cols-5 gap-4 mb-4">
                <div>
                    ...
                    <input name="bi2" type="text" 
                        x-model="form.bi2" 
                        @input="calcularCI(); calcularTotal()"
                        class="border px-2 py-1 w-full rounded">
            ...
            <div>
                ...
                <div>
                    <label class="block mb-1 font-semibold">Importe Retención </label>
...
<script>
function facturasCrud(listaProveedores, listaTipoGasto, listaRetenciones, listaPagos, listaIVA) {
    return {
        modal: false,
        editId: '',
        formTitle: '',
        ...
        cargarTipoGastoProveedor() {
            if (!this.form.proveedor_id) {
                this.form.tipoGasto_id = '';
                return;
            }

            // Buscar el proveedor seleccionado
            let proveedorSeleccionado = this.listaProveedores.find(p => p.id == this.form.proveedor_id);
            if (proveedorSeleccionado && proveedorSeleccionado.tipoGasto_id) {
                this.form.tipoGasto_id = proveedorSeleccionado.tipoGasto_id;
            } else {
                this.form.tipoGasto_id = '';
            }
        },
        ...
        calcularRetencion() {
            if (!this.form.retencion_id || !this.form.bi1) {
                this.form.retencion = '';
                this.calcularTotal();
                return;
            }

            // Buscar la retención seleccionada
            let retencionSeleccionada = this.listaRetenciones.find(r => r.id == this.form.retencion_id);
            ...
            // Obtener el porcentaje de la retención (puede estar como 0.15 para 15% o como 15 para 15%)
            let porcentajeRetencion = parseFloat(retencionSeleccionada.retencion) || 0;
            
            // Si el porcentaje es menor que 1, asumimos que está en formato decimal (0.15), si no, está en formato porcentual (15)
            if (porcentajeRetencion < 1) {
                porcentajeRetencion = porcentajeRetencion * 100; // Convertir 0.15 a 15
            }

            // Calcular: porcentaje_retencion * bi1
            let bi1 = parseFloat(this.form.bi1) || 0;
            let valorRetencion = bi1 * (porcentajeRetencion / 100);
            
            this.form.retencion = valorRetencion.toFixed(2);
            this.calcularTotal();
        },

        calcularTotal() {
            // Calcular CI primero si no está calculado
            this.calcularCI();
            
            let bi1 = parseFloat(this.form.bi1) || 0;
            let ci1 = parseFloat(this.form.ci1) || 0;
            let bi2 = parseFloat(this.form.bi2) || 0;
            let ci2 = parseFloat(this.form.ci2) || 0;
            let bi3 = parseFloat(this.form.bi3) || 0;
            let ci3 = parseFloat(this.form.ci3) || 0;
            let retencion = parseFloat(this.form.retencion) || 0;
            
            // Fórmula: b1 + ci1 + b2 + ci2 + b3 + ci3 - retencion (ya calculada)
            let subtotal = bi1 + ci1 + bi2 + ci2 + bi3 + ci3;
            let total = subtotal - retencion;
            
            this.form.total_factura = total.toFixed(2);
        },
        ...
        openModal(id = '', data = null) {
            ...
            // En modo nuevo, si hay un proveedor seleccionado, cargar su tipoGasto_id
            // En modo edición, solo se cargará si el usuario cambia el proveedor
            if (!id && this.form.proveedor_id) {
                this.cargarTipoGastoProveedor();
            }

            // Calcular CI y Total si hay datos
            if (data && (this.form.bi1 || this.form.bi2 || this.form.bi3)) {
                // Si hay retención seleccionada, calcularla primero
                if (this.form.retencion_id && this.form.bi1) {
