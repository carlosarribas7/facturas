<?php
// facturas.php - Versión "C" (moderna y robusta)
// -------------------------------------------------
// Ajusta estas credenciales antes de usar:
$DB_HOST = '127.0.0.1';
$DB_NAME = 'facturassalt';   // ajusta al nombre real de tu BD
$DB_USER = 'root';
$DB_PASS = '';
$DB_CHARSET = 'utf8mb4';
...
try {
    ...
    echo "<h1>Error de conexión a la base de datos</h1><pre>" . htmlspecialchars($e->getMessage()) . "</pre>";
    exit;
}
...
// ----------------- Endpoints AJAX (save/delete/get) -----------------
// We use POST with action=save|delete|get
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    $action = $_POST['action'];
    if ($action === 'save') {
        // Recogemos datos (normalizar)
        ...
        // Validación servidor (proveedor y tipoGasto son obligatorios según indicaciones)
        ...
        // Calcular CI y total en servidor por seguridad
...
// ----------------- Datos iniciales para renderizar la página -----------------
try {
    $listaProveedores = $pdo->query("SELECT id, nombre FROM proveedores ORDER BY nombre")->fetchAll();
} catch (Exception $e) { $listaProveedores = []; }

try {
    $listaTipoGasto = $pdo->query("SELECT id, cuenta, ce FROM tipo_gasto ORDER BY cuenta")->fetchAll();
} catch (Exception $e) { $listaTipoGasto = []; }

try {
    $listaRetenciones = $pdo->query("SELECT id, retencion, ambito FROM retenciones ORDER BY id")->fetchAll();
} catch (Exception $e) { $listaRetenciones = []; }

try {
    $listaPagos = $pdo->query("SELECT id, nombreMetodoPago FROM pagos ORDER BY id")->fetchAll();
} catch (Exception $e) { $listaPagos = []; }

try {
    $listaIVA = $pdo->query("SELECT id, porcentaje FROM iva ORDER BY porcentaje")->fetchAll();
} catch (Exception $e) { $listaIVA = []; }

// Tabla inicial: obtenemos últimas 50 facturas (puedes ajustar)
...
<head>
  ...
  <!-- Tailwind CDN (rápido para desarrollo) -->
...
<body class="bg-gray-100 p-6">
  <div class="max-w-7xl mx-auto">
    ...
    <!-- Mensaje de servidor (no bloqueante) -->
    ...
    <!-- Contenedor Alpine: le pasamos los datos serializados en un script JSON seguro -->
    ...
    <!-- Modal (Alpine) -->
    <div x-data="{ open:false }" x-show="false" style="display:none;"></div> <!-- fallback to avoid flash -->

    <div x-data="modalData()" x-show="modal" x-cloak>
      <div x-show="modal" x-transition class="fixed inset-0 z-40 flex items-center justify-center modal-backdrop">
...
function facturasApp() {
  return {
    ...
    init() {
      // Inicializamos arrays y tabla
      this.init = this.initData; // keep raw
      this.initRows = this.init.rows || [];
      this.filtered = [...this.initRows];
      this.q = '';
      // Exponer init data a modalData
      document.querySelectorAll('[x-data="modalData()"]').forEach(el => {
        // nothing: modalData reads from initData via function closure
      });
    },
    q: '',
    initRows: [],
    ...
    openAdd() {
      // dispatch to modalData
      window.dispatchEvent(new CustomEvent('open-factura-modal', { detail: null }));
    },
    openEdit(row) {
      window.dispatchEvent(new CustomEvent('open-factura-modal', { detail: row }));
    }
  }
}

/* Modal component factory — reads init-data from the page */
function modalData() {
  ...
  fecha_hora: ''
    ...
    calcAll() {
      ...
      const bi1 = toNum(this.form.bi1); const iva1 = getPercent(this.form.iva1); this.form.ci1 = (bi1 * (iva1/100)).toFixed(2);
      const bi2 = toNum(this.form.bi2); const iva2 = getPercent(this.form.iva2); this.form.ci2 = (bi2 * (iva2/100)).toFixed(2);
      const bi3 = toNum(this.form.bi3); const iva3 = getPercent(this.form.iva3); this.form.ci3 = (bi3 * (iva3/100)).toFixed(2);
    ...
    open(payload) {
      ...
      if (payload && payload.id) {
        ...
        // Map server row fields into form
        Object.keys(this.form).forEach(k => {
          this.form[k] = payload[k] ?? payload[k] === 0 ? payload[k] : (payload[k] === null ? '' : payload[k]);
        });
        // asegurar que porcentajes IVA estén como números (si vienen del servidor)
        this.calcAll();
      } else {
        // nuevo
    ...
    async save() {
      ...
      this.calcAll(); // asegurar total correcto
      ...
      try {
        ...
        if (json.status === 'ok') {
          // actualizar tabla en cliente: si editamos, sustituir; si creamos, añadir al inicio
          if (json.data && json.data.id) {
            // remove old if exists
            const idx = this.findIndexById(json.data.id);
            if (idx >= 0) {
              const root = document.querySelector('[x-data="facturasApp()"]')?.__x;
              if (root && root.get && root.get('initRows')) {
                // simpler: reload page to ensure consistency
                location.reload();
                return;
              }
            } else {
              // nuevo: recargar página (más sencillo y robusto)
              location.reload();
              return;
            }
          } else {
    ...
    findIndexById(id) {
      const rows = JSON.parse(document.getElementById('init-data').textContent || '{}').rows || [];
      return rows.findIndex(r => parseInt(r.id) === parseInt(id));
    },
    confirmDelete(id) {
      ...
      fetch(location.href, { method: 'POST', body: fd })
        ...
        .then(json => {
          if (json.status === 'ok') {
            ...
            alert('No se ha podido eliminar: ' + (json.message || 'error'));
          }
...
/* Wiring: open modal from global buttons */
document.addEventListener('DOMContentLoaded', function() {
  // init add button
  const btn = document.getElementById('btnAdd');
  if (btn) btn.addEventListener('click', () => {
    // trigger modal open for new
