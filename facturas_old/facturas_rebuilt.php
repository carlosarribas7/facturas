...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        if (!isset($_POST['numero_factura']) || trim($_POST['numero_factura']) === '' ||
            !isset($_POST['proveedor_id']) || trim($_POST['proveedor_id']) === '') {
        ...
        if (isset($_POST['id']) && $_POST['id'] !== '') {
            ...
            $sql = "INSERT INTO facturas (proveedor_id, tipoGasto_id, numero_factura, fecha_factura, mes,
                    bi1, iva1, ci1, bi2, iva2, ci2, bi3, iva3, ci3, retencion, retencion_id, total_factura,
                    pago_id, fecha_vencimiento, fecha_banco, fecha_hora)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([
                $data['proveedor_id'], $data['tipoGasto_id'], $data['numero_factura'], $data['fecha_factura'], $data['mes'],
...
if(!empty($_SESSION['error_facturas'])) {
    echo "<div class='bg-red-100 border border-red-300 text-red-800 p-3 rounded mb-4'>"
        . htmlspecialchars($_SESSION['error_facturas'])
        . "</div>";
    unset($_SESSION['error_facturas']);
}
?>
</head>
<body class="bg-gray-100 p-7"
      x-data="facturasCrud(
          <?= htmlspecialchars(json_encode($listaProveedores), ENT_QUOTES, 'UTF-8') ?>,
